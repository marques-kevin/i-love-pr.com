...
const dashboard_slice = createSlice({
  ...
  reducers: {
    ...
    request_import_repo(state, action: PayloadAction<string | null>) {
      state.import_repo_requested = true
      state.import_repo_link = action.payload
    },
    clear_import_repo_request(state) {
