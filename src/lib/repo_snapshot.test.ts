...
describe('repo_snapshot', () => {
  ...
  it('keeps account settings and writes snapshot repo settings to repo_settings', async () => {
    const repo = 'acme/widgets'
    const exporter_repositories = create_memory_repositories({
      settings: base_settings([repo]),
      pull_requests: [sample_pr(repo)],
    })
    await exporter_repositories.repo_settings.save(repo, {
    ...
    const exported = await export_repo_snapshot(exporter_repositories, repo)
  ...
  it('creates empty-token settings when importing with no prior settings row', async () => {
    const repo = 'acme/widgets'
    const snapshot: RepoSnapshotV1 = {
      schema_version: 1,
      exported_at: '2026-01-01T00:00:00.000Z',
