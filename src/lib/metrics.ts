...
import { load_repo_analysis_settings } from './repo_settings'
...
export async function compute_metrics(options: {
  ...
  const settings_by_repo = await load_repo_analysis_settings(repositories, repos)
  ...
  if (hide_test_files && prs.length > 0) {
    ...
    prs = prs.map((pr) => {
      ...
      const test_file_globs =
        settings_by_repo.get(pr.repo_full_name)?.test_file_globs ?? DEFAULT_TEST_FILE_GLOBS
  ...
  const reviews = all_reviews.filter((r) => {
    ...
    if (!pr) return false
    const ignored_bots = settings_by_repo.get(pr.repo_full_name)?.ignored_bots ?? []
...
export async function list_contributors(
  ...
  for (const r of reviews) {
    const ignored_bots = settings_by_repo.get(r.repo_full_name)?.ignored_bots ?? []
