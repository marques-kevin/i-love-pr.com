  ...
  return (
    <AnimatePresence>
      {show && items.length > 0 && (
        ...
        >
          {heading && (
            <div className="mb-0.5 font-pixel text-[10px] text-muted-foreground">
              {heading}
            </div>
          )}
          <div className="flex flex-col gap-0.5">
            {items.map((item) => (
              <div
                key={item.name}
                className="flex items-center gap-1.5 font-pixel text-[11px] text-popover-foreground tabular-nums"
